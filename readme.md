...
Since the .docx files in the original project assumed the familiarity with DirectX12, the following document will show also some necessary information for those who, like me, had no knowledge about DirectX12 before starting to work on it and to give a better understanding of the changes applied in the "*dev*" branch of this project. This purpose will be achieved through a brief explanation of the main DX12 structures with links leading directly to the specific topic covered in the DirectX documentation, then, it will follow a presentation of what has been added and modified in this project.
...
- **Descriptor-heap**: starting with DirectX12, **RTV** (**render target view**) are stored in this data structure, where a view references a resource in the GPU memory.
  Here for more: https://www.3dgep.com/learning-directx-12-1/#preamble; 
- **Fence**: for synchronising commands sent to the command queue.
  Here for more: https://www.3dgep.com/learning-directx-12-1/#fence;

It is worth noting that the **any-hit shader**, which only runs with non-opaque intersections, is not even launched because of the ```D3D12_RAYTRACING_GEOMETRY_FLAG_OPAQUE``` flag used in the acceleration structure which sets both geometries to opaque.

Now that the main topics used in this DirectX project have been shown, we can move on to the core of the changes made, looking at the final result and the operations required to achieve it, as all the work can be seen in the previous commits.

The scope of this project was to realise a scene where the instances in the background and in the foreground would be coloured according to some sort of *transparency* factor.
In order to do this, more instances had to be added, so the **Acceleration Structures** had to be modified in terms of the **Constant Buffers** used for each instance and the ```NumDescs``` parameter which had to be changed depending on the number of triangle instances in the scene. Then, more triangle instances were added in order to create a scene with three instances placed in the foreground and the remaining three placed in the background. To do this, also the **Shader Table** had to be corrected using the following scheme: 
...
- the use of two different rays crossing distinct instances depending on the properties of these latter:
  ...
  - a second ray crosses the remaining instances without the transparency factor where the former have been still defined with its own payload as well;
...
- ```InstanceInclusionMask``` has different values depending on the instance and ray types:
  ...
  - ```0x80``` standing for **"RAY_FLAG_CULL_NON_OPAQUE"** which causes the ray to ignore the instances marked with the "NON_OPAQUE" flag, i.e. all the instances which are to consider having the *transparency* factor;
  - ```0x40``` standing for **"RAY_FLAG_CULL_OPAQUE"** that, on the contrary, causes the ray to consider only the instances marked with the "OPAQUE" flag, i.e. the only instances that do not have the *transparency* factor;
...
- ```RayPayload``` has different properties depending on the ray type and this is the **key attribute** used to achieve the goal. Its structure is defined as follows:
  ...
  ``` 
  while, for the instances to consider the transaparency factor:
  ```
  ...
  ```
    ...
    ```
Subsequently, considering what described until now, the **blending** was tuned consequently through the following scheme in the shader file, more specifically in the ```triangleClosestHitShader```:
```
...
- if the ray crosses the "NOT_OPAQUE" instances -> the considered payload is the ```transparencyPayload``` -> only the *transparent* instances are coloured according to the blending defined after the first condition;
- if the ray traverses the "OPAQUE" instances -> the considered payload becomes the ```NoTransPayload``` -> only the *not transparent* instances are colored following the blending defined in the "else" condition; 
...
Another required change in order to add triangle instances in the scene was to adjust the correct memory locations in the shader table, recalculating the **Shader Table entry size** and mapping each buffer in this table.
...
Finally, in the "Video" folder you will find a video demonstration of what has been described so far and you will find below some useful references that helped me to get the correct information required to realise this project:
